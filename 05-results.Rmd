...
dataset %>% ggplot() + aes(x = FREQ1, fill = GENDER) + geom_histogram(alpha =0.3, stat = "count",position = "identity") + theme_bw()
...
cases_per_year_for_a_region_and_state_drug %>% 
  ...
  ggplot(aes(state = state,fill = value)) + geom_statebins() + facet_wrap(~Year) + theme_bw() + ggtitle("Proportion of admissions for state across time")+ labs(subtitle = 'Substance: Heroin') + theme(axis.text.x = element_blank(),axis.text.y = element_blank(),axis.ticks = element_blank()) + scale_fill_viridis(option="mako" ,name = "Proportion of Admission", direction = -1) + theme(legend.position="top")
...
```{r fig.height=8, fig.width=6,echo=FALSE,warning=FALSE,message=FALSE,error=FALSE, results='hide',fig.keep='all'}

dataset %>% ggplot() + aes(x = SERVICES, fill = SUB1)+ geom_histogram(alpha =0.3,
                 stat = "count",
                 position = "dodge") + theme_bw() + theme(axis.text.x = element_text(angle = 60, vjust = 0.5), plot.title = element_text(hjust = 0.5, size = 10)
                 ) + facet_wrap(~ REGION)
...
```{r gender,fig.height = 6,fig.width = 10,echo=FALSE,warning=FALSE,message=FALSE,error=FALSE, results='hide',fig.keep='all'}
...
ggplot(
  ...
  aes(x = STFIPS , y = Proportion_of_cases, fill = GENDER)) + geom_bar(stat = "identity",
             width = .6,
             position = "dodge")   +coord_flip() +  theme_bw() + facet_wrap( ~ ADMYR) + ylab("States")+xlab("Proportion of cases")+ggtitle("Proportion of admission for each gender and sate facted by years") + labs(subtitle = 'Substance: Heroin') + theme(legend.position="top")
...
cases_per_year_for_a_region_and_state_and_age_drug <- cases_per_year_for_a_region_and_state_and_age_drug %>% mutate( AGE_CATEGORY = case_when(
  AGE == "12–14" ~ "12–17",
  AGE == "15–17" ~ "12–17",
  AGE == "18–20" ~ "18–24",
  AGE == "21–24" ~ "18–24",
...
cases_per_year_for_a_region_and_state_and_age_drug <- cases_per_year_for_a_region_and_state_and_age_drug %>% group_by(ADMYR,SUB1,REGION,STFIPS,AGE_CATEGORY) %>% summarise(Total_cases = sum(`Total_cases`),Total_population = sum(`Total_population`))
...
ggplot(cases_per_year_for_a_region_and_state_and_age_drug %>% filter(REGION == 'South',SUB1 == 'Heroin'), aes(fill=AGE_CATEGORY, y=Proportion_of_cases, x=STFIPS, label = Proportion_of_cases)) + theme_bw() + scale_y_continuous(expand = c(0, 0), breaks = pretty_breaks()) +  theme(text = element_text(size=14)) +
...
ggplot(data = tmpds2 %>% filter(EMPLOY != 'Missing')) + geom_mosaic(aes(x=product(STFIPS,SUB1), fill = EMPLOY), divider=ddecker()) + scale_alpha_manual(values =c(.7,.9)) + theme(axis.text=element_text(size=12),axis.text.x = element_text(angle = 90, hjust = 1, vjust = .5),plot.title = element_text(hjust = 0.5,size=20), strip.text = element_text(size=13), legend.key.width = unit(2.5, "cm"),legend.key.height = unit(1.5, "cm"), legend.key.size = unit(0.8, "cm")) + labs(title="Substance and States vs Employment status") + ylab("Employment Status")+xlab("States & substance") + theme_hc() +  theme(legend.position="right") + guides(fill = guide_legend(reverse = TRUE,title="Employment Status"))
```
...
```{r race,fig.height = 4,fig.width = 8,echo=FALSE,warning=FALSE,message=FALSE,error=FALSE, results='hide',fig.keep='all'}
...
ggplot(data = dataset_sub_filtered_edu_employ) + geom_mosaic(aes(x=product(SUB1,EDUC), fill = AGE), divider=ddecker()) + scale_alpha_manual(values =c(.7,.9)) + ggtitle("Impact of Education and Age on the Drugs") + theme(axis.text=element_text(size=12),axis.text.x = element_text(angle = 90, hjust = 1, vjust = .5),plot.title = element_text(hjust = 0.5,size=20), strip.text = element_text(size=13), legend.key.width = unit(2.5, "cm"),legend.key.height = unit(1.5, "cm"), legend.key.size = unit(0.8, "cm")) + theme_hc()
