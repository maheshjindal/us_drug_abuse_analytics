...
cases_per_year_for_a_region_and_state_drug_filtered <- cases_per_year_for_a_region_and_state_drug %>% filter(abbrev != 'MD')%>% filter(abbrev != 'DE')
ggplot(data = cases_per_year_for_a_region_and_state_drug_filtered[which(cases_per_year_for_a_region_and_state_drug$SUB1 == 'Heroin'),],mapping = aes(x = ADMYR, y = Proportion_of_cases)) +
  geom_line(mapping = aes(group = STFIPS), alpha = 0.5) + 
  geom_smooth(formula = y ~ x, method = "loess",mapping = aes(group = REGION),se = FALSE) + 
  geom_text_repel(data =subset(cases_per_year_for_a_region_and_state_drug_filtered[which(cases_per_year_for_a_region_and_state_drug$SUB1 == 'Heroin'),],ADMYR == '2019'),mapping = aes(x = ADMYR, y = Proportion_of_cases, label = abbrev),size = 3, segment.color = NA, nudge_x = 30) + facet_wrap(~REGION, nrow  = 3) + theme_bw() + ylab("Proportion of cases")+xlab("Year")+ggtitle("Year vs Proportions of admissions for each state faceted by region") + labs(subtitle = 'Substance: Heroin') + theme(axis.text.x = element_text(angle = 60, hjust = 1, vjust = .5))
```
Observations:
...
ggplot(data = tmpds2 %>% filter(EMPLOY != 'Missing')) + geom_mosaic(aes(x=product(STFIPS,SUB1), fill = EMPLOY), divider=ddecker()) + scale_alpha_manual(values =c(.7,.9)) + theme(axis.text=element_text(size=12),axis.text.x = element_text(angle = 90, hjust = 1, vjust = .5),plot.title = element_text(hjust = 0.5), strip.text = element_text(size=13)) + labs(title="Substance and States vs Employment status") + ylab("Employment Status")+xlab("States & substance") + theme_hc() +  theme(axis.text.x = element_text(angle = 60, hjust = 1, vjust = .5), legend.position="right") + guides(fill = guide_legend(reverse = TRUE,title="Employment Status"))
```
